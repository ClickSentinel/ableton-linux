...
1. Prepares an empty Wine prefix at `~/wires/plugs/studio`. A non-empty prefix
...
| `--prefix DIR` | Test prefix; default `~/wires/plugs/studio` |
...
The kit checks known paths under `~/wires` and
...
```bash
WIRES_RUNTIME_SOURCE=/path/to/wine-d2d1-nspa-src \
