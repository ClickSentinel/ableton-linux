...
configured Wine installer, prepares `~/wires/plugs/studio`, runs the probes, and
writes:
...
After installing Live in `~/wires/plugs/studio`, start it and run the Live checks:

```bash
./beta/tester-kit/run-session --live-only \
  --wine "$(wires runtime path)/bin/wine"
```
