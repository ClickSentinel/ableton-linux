...
| --- | --- | --- |
| `patch-stack.bats` | The 53-patch Wine series and the 2 PipeASIO patches: applies them all to the vendored base, checks the freeze manifest, audit registration, and numbering | A rotted patch is otherwise caught only by the ~40-minute container build, and only on PRs touching the paths `ci-pr-build.yml` filters on |
...
| `release.bats` | Release-time invariants: VERSION against the top CHANGELOG entry, the committed BUILD-INFO and its patch count, and the README's download link | The only suite that does not run on a PR. Its checks are true of a tagged release and false at legitimate points in between — e221cc4 landed a changelog entry while VERSION still read the previous release — so `release.yml`'s `draft` job runs it on the `v*` tag instead |
