...
**The launcher is run, not sourced.** `scripts/ableton-live` does discovery, locking and registry sync at top level and ends in `exec wine`. `helpers/launcher.bash` runs the whole thing against a throwaway `$HOME` and prefix with `WIRES_RUNTIME` pointed at a fake runtime whose `wine` logs its argv and exits — so a test asserts on the command line the launcher *would* have run. Its pure functions are extracted by name and evaluated separately, since a full launch says nothing about colour arithmetic.
