...
[bats-core](https://github.com/bats-core/bats-core) is the harness. `run.sh` uses `bats` from `PATH`, or `$BATS`, or clones a pinned copy into `tests/.bats-core` on first use.
