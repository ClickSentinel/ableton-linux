...
setup() {
    ...
    export WIRES_PROMOTE_DEST="$BATS_TEST_TMPDIR/out"
}
...
@test "a nightly promotes: plain name, checksum, and both BUILD-INFO copies" {
    ...
    [ "$status" -eq 0 ]
    [ -f "$WIRES_PROMOTE_DEST/$NAME-2026.08.09.1.tar.zst" ]
    [ -f "$WIRES_PROMOTE_DEST/$NAME-2026.08.09.1.tar.zst.sha256" ]
    [ -f "$WIRES_PROMOTE_DEST/BUILD-INFO-2026.08.09.1.txt" ]
    [ -f "$WIRES_PROMOTE_DEST/BUILD-INFO.txt" ]
    ( cd "$WIRES_PROMOTE_DEST" && sha256sum -c "$NAME-2026.08.09.1.tar.zst.sha256" )
}
...
@test "the output is a tarball the selector accepts as a release" {
    ...
    wires_is_runtime_tarball "$WIRES_PROMOTE_DEST/$NAME-2026.08.09.1.tar.zst"
    [ "$(basename "$(wires_pick_tarball "$WIRES_PROMOTE_DEST")")" = "$NAME-2026.08.09.1.tar.zst" ]
}

@test "dist-version is the release, build-kind is gone, promoted-from records the origin" {
    mk_nightly
    bash "$REPO/scripts/promote-nightly.sh" "$TARBALL" 2026.08.09.1 >/dev/null
    info="$WIRES_PROMOTE_DEST/BUILD-INFO-2026.08.09.1.txt"
    [ "$(wires_buildinfo_field "$info" dist-version)" = "2026.08.09.1" ]
    [ -z "$(wires_buildinfo_field "$info" build-kind)" ]
    [ "$(wires_buildinfo_field "$info" promoted-from)" = "2026.08.06.1+nightly.79d8960" ]
}
...
@test "source-commit and built-at survive the restamp unchanged" {
    ...
    bash "$REPO/scripts/promote-nightly.sh" "$TARBALL" 2026.08.09.1 >/dev/null
    info="$WIRES_PROMOTE_DEST/BUILD-INFO-2026.08.09.1.txt"
    [ "$(wires_buildinfo_field "$info" source-commit)" = "79d8960aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" ]
    [ "$(wires_buildinfo_field "$info" built-at)" = "2026-08-06T04:12:00Z" ]
}
...
@test "the payload bits survive verbatim" {
    ...
    bash "$REPO/scripts/promote-nightly.sh" "$TARBALL" 2026.08.09.1 >/dev/null
    after="$(zstd -dc "$WIRES_PROMOTE_DEST/$NAME-2026.08.09.1.tar.zst" | tar -xO "$NAME/bin/wine" | sha256sum)"
    [ "$before" = "$after" ]
}
...
@test "the unpacked result names itself <version>+<short-commit> in the store" {
    ...
    zstd -dc "$WIRES_PROMOTE_DEST/$NAME-2026.08.09.1.tar.zst" | tar -x -C "$d"
    [ "$(wires_runtime_id "$d/$NAME")" = "2026.08.09.1+79d8960" ]
}
...
# WIRES_TEST_TARBALL convention in install-runs.bats.
@test "a real nightly tarball promotes losslessly" {
    [ -n "${WIRES_TEST_NIGHTLY_TARBALL:-}" ] && [ -f "$WIRES_TEST_NIGHTLY_TARBALL" ] \
        || skip "no nightly tarball; set WIRES_TEST_NIGHTLY_TARBALL to run this"
    before="$(zstd -dc --long=27 "$WIRES_TEST_NIGHTLY_TARBALL" | tar -xO "$NAME/bin/wine" | sha256sum)"
    bash "$REPO/scripts/promote-nightly.sh" "$WIRES_TEST_NIGHTLY_TARBALL" 2026.08.09.1 >/dev/null
    out="$WIRES_PROMOTE_DEST/$NAME-2026.08.09.1.tar.zst"
