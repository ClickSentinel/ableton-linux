...
published() {   # commit, [runtime-name], [sha-override]
    ...
    local name="${2:-wires-2026.09.01.1.tar.zst}"
...
@test "a checksum mismatch refuses before anything is staged" {
    setup_stubs; stub pgrep 1
    published aaaaaaaa wires-2026.09.01.1.tar.zst notthesha
    run bash "$REPO/wires/wires-runtime" install
    [ "$status" -ne 0 ]
...
@test "a named tarball skips the web entirely" {
    ...
    published aaaaaaaa
    run bash "$REPO/wires/wires-runtime" install "$PUB/wires-2026.09.01.1.tar.zst"
    [ "$status" -eq 0 ] || { echo "$output" >&2; false; }
    [[ "$output" != *"channel:"* ]]
    [ -L "$C/stable" ]
}
