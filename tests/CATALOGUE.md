...
392 tests across 16 suites. See [README.md](README.md) for how to run
...
- [tests/unit/works.bats](#works) — 10 test(s)
- [tests/unit/works-runtime.bats](#works-runtime) — 32 test(s)
- [tests/unit/works-plug.bats](#works-plug) — 37 test(s)
...
| 2 | an id collision sets the old tree aside instead of nesting it in the entry | found in review. The primary path ended in a bare `mv` while both |
...
| 4 | update is delegated | — |
| 5 | stop is delegated to the runtime verb | `works stop` is the documented spelling and has to arrive at the same |
| 6 | no command at all prints the usage | — |
| 7 | an unknown command is refused, with the usage to hand | — |
...
| 9 | help ends on a command, not on prose | the help is a fixed line range over the header comment, so editing that |
| 10 | help names every command it dispatches | — |
...
| 10 | use refuses a Plug that is not there, and lists what is | — |
| 11 | default is refused as a Plug name | `default` is the selection link itself, so a Plug by that name could |
| 12 | a name that is not a plain directory name is refused | — |
| 13 | new creates a Plug that follows the channel | — |
| 14 | a Plug created but never booted is still listed | a Plug with no prefix in it yet still has to appear, or `new` produces |
| 15 | new refuses a name already taken | — |
| 16 | new --runtime pins the Plug to one build | — |
| 17 | new --runtime refuses a build that is not in the store | — |
| 18 | new --from clones the prefix and what is installed in it | — |
| 19 | cloning keeps the prefix's symlinks as symlinks | dosdevices holds relative links back into the Plug and outward ones to |
| 20 | a clone inherits the binding the source had | — |
| 21 | new --from refuses a source that is not a Plug | — |
| 22 | new says which kind of copy it is about to make | — |
| 23 | the clone names the filesystem the mount table reports | `stat -f` reads statfs.f_type, and ext2, ext3 and ext4 all share magic |
| 24 | new --from succeeds on a machine with no version store | found by hand on a pre-store machine. The clone landed, the *default* |
| 25 | an explicit --runtime is refused before anything is cloned | an explicit --runtime is a different case from the default binding, and |
| 26 | rm -y removes the Plug and everything in it | — |
| 27 | rm refuses the default Plug while others exist, and names the successors | removing what default points at leaves the selection dangling and |
| 28 | removing the last Plug takes the default link with it | the guard above cannot fire for the last Plug, and leaving the link |
| 29 | rm without -y and with no terminal refuses rather than assuming | this deletes a prefix that can hold a licensed Live and tens of GB of |
| 30 | rm refuses a Plug that is not there | — |
| 31 | a Plug's binding decides the runtime a launch binds to | two Plugs running different builds is the whole point of the binding, |
| 32 | an unbound Plug binds to whatever the channel resolves to | — |
| 33 | WORKS_RUNTIME still overrides a Plug's binding | the VMs and anyone bisecting a build rely on WORKS_RUNTIME being the |
| 34 | retention keeps a build a Plug is bound to | a Plug held deliberately on an older build is exactly what the count |
| 35 | retention prunes that same build when no Plug is bound to it | — |
| 36 | help ends on a command, not on prose | — |
| 37 | plug is reachable through the dispatcher | — |
...
| `found in review. The primary path ended in a bare `mv` while both` | migrate-layout: an id collision sets the old tree aside instead of nesting it in the entry |
...
| `the help is a fixed line range over the header comment, so editing that` | works-update: update help ends on a command, not on prose<br>works: help ends on a command, not on prose |
