...
# ~/wires/apps/ableton-live/ableton-linkd); this script never builds software.
#
...
linkd="${ABLETON_LINKD:-$HOME/wires/apps/ableton-live/ableton-linkd}"
marker="$HOME/wires/apps/ableton-live/link-configured"
...
if [ ! -x "$linkd" ]; then
    ...
    if [ ! -f "$unit_src" ]; then
        ...
        if ! command -v systemctl >/dev/null 2>&1; then
            ...
            if [ "$prior_version" -lt 5 ] \
               && systemctl --user is-enabled --quiet ableton-linkd.service 2>/dev/null; then
                if systemctl --user disable --now ableton-linkd.service >/dev/null 2>&1; then
                    ...
                    echo "!! then re-run ~/wires/apps/ableton-live/setup-link.sh" >&2
                    exit 1
                fi
            fi
        else
            ...
            echo "!! log in on the machine itself and run ~/wires/apps/ableton-live/setup-link.sh again" >&2
            exit 1
        fi
    fi
fi
...
if [ "$anchor" = session ] && [ "$service_checked" -eq 1 ]; then
    mkdir -p "$HOME/wires/apps/ableton-live"
