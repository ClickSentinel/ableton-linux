...
VERSION="$(cat VERSION)"
# Which runtime goes into the kit. ABLETON_RUNTIME_TARBALL pins one outright,
# which is what promoting a build to a release consists of: re-wrapping a
# tarball this machine did not produce. Otherwise the exact-version runtime if
# present, else the newest properly-named one — never a bare glob, which would
# pick the -debug tree whenever both are in dist/.
