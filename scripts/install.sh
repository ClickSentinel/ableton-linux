...
# the dated rollbacks and the still-running check all follow the target, so an
# overridden root gets the same safety net as the default one.
...
if pgrep -af '[A]bleton Live.*\.exe|[P]ush2DisplayProcess.exe' >/dev/null 2>&1 || \
   pgrep -af "$WINE_ROOT" >/dev/null 2>&1; then
