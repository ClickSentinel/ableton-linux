...
Publishing flips `/releases/latest`, makes the manifest live for `ableton-update`, and runs `release.yml`'s verify job over every asset. A draft that fails the human check is deleted, not published; the tag can be reused after `git push --delete origin v<VERSION>`.
