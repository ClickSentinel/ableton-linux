...
```bash
"$HOME/wires/apps/ableton-live/setup-link.sh"
```
...
```text
~/wires/apps/ableton-live/ableton-linkd
```
...
```bash
"$HOME/wires/apps/ableton-live/ableton-linkd" --daemon
"$HOME/wires/apps/ableton-live/ableton-linkd" --probe 10
```
...
```bash
env WINEPREFIX="$HOME/wires/plugs/studio" \
  "$(wires runtime path)/bin/wine" tools/linkprobe.exe
```
