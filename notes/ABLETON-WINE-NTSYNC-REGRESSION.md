...
used a reflink clone of `~/wires/plugs/studio` with its own wineserver.
...
```bash
cp -a --reflink=auto "$HOME/wires/plugs/studio" "$HOME/wires/plugs/studio-ntsync-test"
WIRES_PLUG="$HOME/wires/plugs/studio-ntsync-test" \
  WIRES_RUNTIME="/path/to/runtime" \
  ./scripts/check-ntsync.sh
rm -rf -- "$HOME/wires/plugs/studio-ntsync-test"
```
