...
Hovering a control in a Max for Live device UI pops a tooltip several hundred pixels away, clustered at one spot while the pointer is elsewhere. It reads as random because the spot changes between bursts as the device UI relayouts.

No Wine-side cause was found across seven tested hypotheses, each refuted with a recorded experiment. The evidence points at Max/JUCE computing the wrong coordinate before any Win32 call: Wine honours every positioning request exactly, 24 times out of 24.

That is a negative result, not a proof of absence. This ruled out only the paths it thought to check — those below, plus the mechanisms listed with them. A Wine-side cause *upstream* of the position Max computes has not been exhaustively excluded; the query APIs actually examined were `GetCursorPos`, `GetMonitorInfo`/`SPI_GETWORKAREA` and `GetWindowRect`. A new idea for what feeds that computation is worth testing, but check it against the refutations first.

A separate and genuine Wine bug surfaced along the way, under [Cursor position](#cursor-position). It does not cause this symptom.

## Symptom

Right-click context menus are not affected. Every `title="menu"` JUCE menu and every win32u `#32768` menu observed across five captures landed correctly at the cursor. Only the small hover tooltips misplace.

If someone reports "menus appear in random places", confirm which of the two they mean before investigating. That ambiguity cost several captures here.
...
Drop-shadow slivers appear alongside as `200x12` / `12x106` windows with `WS_EX_TRANSPARENT` added. They move with the tooltip and are not themselves interesting.
...
The side monitors start at `y=360` and the primary at `y=0`, so the desktop has a notch across the top. Investigated as a cause and ruled out.

## Evidence

Wine honours every positioning request exactly. Parsing `calc_winpos` from a `+win` trace and comparing the requested `swp` against the resulting `new_rects`:
...
The application repositions the tooltip itself. With `+relay,+win` together the sequence per tooltip is unambiguous:
...
The destination is right-aligned to a fixed rect, not the pointer. Widths vary and `left` adjusts to keep `right` constant:
...
Three clusters, each with a constant top (`368`, `320`, `560`). That is placement against a rect, and the rect is not any window on screen.

It is not derived from the pointer: sampling `GetCursorPos` at 50ms alongside a `+win` capture, only 5 of 30 destinations fall within 40px of any reported cursor position, and those look coincidental. It is not derived from the device window either: in one capture that window moved from `(822,374)-(1700,668)` to `(745,459)-(1623,753)` mid-session while destinations did not track it.
...
| 4 | The extra floating Max device window confuses hover routing | closed it; still misplaced |
...
Possible direction: have `winex11.drv` fall back to the last position from motion events when `XQueryPointer` cannot give a trustworthy global answer. Needs its own investigation and repro; do not fold it into the tooltip issue.
...
| `popupplace` | creates popups with chosen style and creation-sequence combinations at a known position and reports drift — isolates which style bit makes Wine hand placement to the WM |
...
| `curprobe` | samples `GetCursorPos` and the foreground window, flags physically impossible jumps |
...
These cost most of the eight captures this took. Read before writing another probe.
...
2. Reporting after a fixed settle delay catches popups post-teardown instead. Max parks a dismissed popup at a fixed anchor *and* collapses it to a sliver (300px wide to 16px). Distinguish by size: a same-size move is a real misplacement, a move that also resizes is teardown. Conflating these produced a wrong "all menus were placed correctly" verdict.
3. Do not mark a window seen before checking visibility. Windows that exist but are hidden at probe startup get suppressed forever and never appear when shown. This hid the Max device window from three captures despite it being a popup's owner.
4. Exclude the popup itself from any anchor search, or every popup matches its own rect and the report is worthless.
...
6. Never pipe a long capture into `grep`. Buffered output is lost when the process is killed; an early attempt produced 7 lines instead of about 1M. Redirect to a file and filter afterwards.
